...
class bsSelect {
	...
	#sortWrapper(parameters, parent = 0) {
		...
		if (parent != 0 && list && list.querySelector("hr")) {
			let unchanged = []
			for (let pile of Object.keys(this.optionPiles[parent])) {
		...
		for (let [index, option] of options.entries()) {
			...
			if (!!(option && option.nodeType === Node.ELEMENT_NODE && ["OPTION", "OPTGROUP"].includes(option.tagName))) {
				...
				if (option.tagName === "OPTION") {
					...
					if (this.multiple) {
						item = '<div id="select-option-group-wrapper-' + this.seq + '-' + rnd + '" class="select-option-group-wrapper' + itemDisabled + '"><div class="form-check"><input type="checkbox" id="select-option-group-checkbox-' + this.seq + '-' + rnd + '" class="form-check-input select-option-group-checkbox" value="' + rnd + '"' + itemDisabled + '><label class="form-check-label select-option-group-label" for="select-option-group-checkbox-' + this.seq + '-' + rnd + '"><div id="select-option-group-' + this.seq + '-' + rnd + '" class="select-option-group select-option-group-' + this.seq + itemDisabled + '" role="optgroup"><span class="select-option-group-text-wrapper">' + icon + '<span class="select-option-group-text">' + option.label + '</span></span>' + image + '</div></label>' + comment + '</div><div id="select-option-group-children-' + this.seq + '-' + rnd + '" class="select-option-group-children"></div></div>'
					}
					else {
